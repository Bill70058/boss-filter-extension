...
> 基于 DeepSeek AI，在 BOSS直聘搜索页/推荐页快速筛选匹配候选人，并在聊天页支持未读消息自动回复
...
1. 打开聊天页 `https://www.zhipin.com/web/chat/index`
2. 点击插件图标，在「设置-自动回复」添加回复预设（岗位/关键词/回复内容）
3. 可选：开启“自动回复”，设置扫描间隔与模式（AI/固定话术）
4. 点击「开始AI筛选」或「立即执行自动回复」进行未读消息回复
...
| 自动回复开关 | 开启后在聊天页可按间隔扫描未读消息 |
| 自动回复间隔 | 扫描未读消息的时间间隔（分钟） |
| 自动回复模式 | AI 生成或固定话术 |
| 自动回复预设 | 按岗位/关键词命中后回复对应话术 |
...
**Q：提示“当前在聊天页，请使用自动回复功能”但我在筛选页？**
A：当前聊天页识别仅以 `https://www.zhipin.com/web/chat/index` 为准；若仍提示请反馈实际页面 URL。
